...
这组文档把产品叙事拆成六个可独立设计、实现和验收的机制。文档以当前仓库实现为基线，同时明确哪些行为已经存在、哪些仍属于产品方向，以及后续工程化时需要补齐的边界。
...
```text
React / Desktop / Web / Mobile
              │ HTTP + WebSocket
              ▼
        Express API + Redis bus
              │
       ┌──────┴────────┐
       ▼               ▼
...
- 代码路径均以仓库根目录为基准；优先阅读服务端实现，再对照前端展示。
