        ...
        <aside className={cn('h-full overflow-y-auto border-r border-ink-100 bg-cloud', compact && 'absolute inset-0 border-r-0')}>
          ...
          {overview.dueReadbacks.length > 0 && (
            <div className="mx-3 mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5">
              <div className="text-[11px] font-bold text-amber-800">{overview.dueReadbacks.length} {translate("production readback")}{overview.dueReadbacks.length === 1 ? '' : 's'} {translate("due")}</div>
            ...
            {overview.features.map((feature) => {
              ...
              return (
                <button key={feature.id} onClick={() => void select(feature.id)} className={cn('mb-1 w-full rounded-xl px-3 py-3 text-left transition', selectedId === feature.id ? 'bg-sky2-50 ring-1 ring-sky2-200' : 'hover:bg-ink-50')}>
                  ...
                  <div className="mt-1.5 flex justify-between text-[10px] text-ink-400"><span>{feature.passedSquares}/{feature.requiredSquares} {translate("required squares")}</span><span>{feature.riskLevel} {translate("risk")}</span></div>
                </button>
              )
            })}
...
function ContractSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="01 · Define" title={translate("Feature contract")} action={<Button tone="primary" disabled={saving} onClick={() => runAction(update(form), () => event('shipping_contract_saved'))}>{translate("Save contract")}</Button>}>
...
function InvariantsSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="02 · Constrain" title={translate("Invariants")} action={<div className="text-[10px] text-ink-400">{translate("What must remain true")}</div>}>
...
function VerificationsSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="03 · Verify" title={translate("Independent evidence squares")} action={<div className="text-[10px] text-ink-400">{translate("Builder ≠ verifier · evidence required")}</div>}>
...
function ReleaseSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="04 · Release" title={translate("Ignition, smoke, and production readback")} action={<Button onClick={() => setShow((v) => !v)}>{show ? 'Close planner' : 'Plan release'}</Button>}>
...
function LearningSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="05 · Learn" title={translate("Friction and regression memory")}>
